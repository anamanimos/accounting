...
class App_Model extends CI_Model
{
	...
	{
		// $u = mysqli_real_escape_string($this->conn_id, $usr);
		// $p = md5(mysqli_real_escape_string($this->conn_id, $psw));
		$u = $usr;
		$p = md5($psw);
		$q_cek_login = $this->db->get_where('users', array('username' => $u, 'password' => $p));
		// $q_cek_login = $this->db->get_where('users', array('username' => 'admin', 'password' => 'admin'));
